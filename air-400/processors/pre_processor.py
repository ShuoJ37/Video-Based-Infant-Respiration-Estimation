...
class PreProcessor:
    ...
    def _load_params(self) -> None:
        ...
        # Add GLSA parameteres
        self.do_pose_guided_glsa = self.preprocess_config.get('DO_POSE_GUIDED_GLSA', False)
        self.glsa_grid_size = self.preprocess_config.get('GLSA_GRID_SIZE', [4, 4])
        self.glsa_base_weight = float(self.preprocess_config.get('GLSA_BASE_WEIGHT', 0.35))
        self.glsa_torso_weight = float(self.preprocess_config.get('GLSA_TORSO_WEIGHT', 1.0))
        self.glsa_pose_conf_threshold = float(self.preprocess_config.get('GLSA_POSE_CONF_THRESHOLD', 0.30))
        self.glsa_pose_frequency = int(self.preprocess_config.get('GLSA_POSE_FREQUENCY', 30))
        
        
        # Dynamic torso padding for GLSA mask.
        # If all four torso(L/R hip and shoulder) are confident use GLSA_FOUR_JOINT_PAD
        # If one same side shoulder and hip pair are confident then use GLSA_SAME_SIDE_PAD
        self.glsa_four_joint_pad = float(self.preprocess_config.get('GLSA_FOUR_JOINT_PAD', 0.50))
        self.glsa_same_side_pad = float(self.preprocess_config.get('GLSA_SAME_SIDE_PAD', 0.10))

        self.pose_repo_path = self.preprocess_config.get('POSE_REPO_PATH', '')
        self.pose_cfg_file = self.preprocess_config.get('POSE_CFG_FILE', '')
        self.pose_model_path = self.preprocess_config.get('POSE_MODEL_PATH', '')
    ...
    def _init_detector(self) -> None:
        ...
        self.pose_cfg = None
        self.pose_get_affine_transform = None
        if getattr(self, 'do_pose_guided_glsa', False):
    ...
    def preprocess(
        ...
        #
        # Important: this follows the original baseline design.
        # If DO_CROP_INFANT_REGION=True, frames have already been cropped by YOLO
        # or the baseline body/face/chest ROI logic above. Optical flow is computed
        # only on those cropped frames, not on the full original image.
        #
        # GLSA does not run a second YOLO. It uses FiDIP on the same cropped frames
        # to build a grid mask, then applies that mask to the single optical-flow result.
        flow = None
        glsa_mask = None
        if self.do_optical_flow:
            # Resample if required. Keep frames and labels aligned before both
            # FiDIP/GLSA and optical flow.
            if self.of_resample_fs > 0 and self.of_resample_fs != raw_fs:
                frames, labels = self._resample_frames_labels(frames, labels, raw_fs)

            if self.do_pose_guided_glsa:
                print("===== GLSA OPTION B ACTIVE: FiDIP mask on cropped ROI =====")
                glsa_mask = self.build_pose_guided_glsa_mask(frames)
            else:
                print("===== BASELINE OPTICAL FLOW =====")

            # Single optical-flow computation on the cropped ROI.
            flow = self.run_optical_flow(frames)

            # Apply the GLSA mask to the same optical flow. If FiDIP failed,
            # glsa_mask is all ones, so this is exactly baseline optical flow.
            if self.do_pose_guided_glsa:
                flow = self.apply_glsa_mask_to_flow(flow, glsa_mask)
    ...
    def hash_param_dict(self) -> Tuple[Dict[str, Any], str]:
        ...
        preprocess_param_dict = {
            ...
            'DO_POSE_GUIDED_GLSA': self.do_pose_guided_glsa,
            'GLSA_GRID_SIZE': self.glsa_grid_size,
            'GLSA_BASE_WEIGHT': self.glsa_base_weight,
            'GLSA_TORSO_WEIGHT': self.glsa_torso_weight,
            'GLSA_POSE_CONF_THRESHOLD': self.glsa_pose_conf_threshold,
            'GLSA_POSE_FREQUENCY': self.glsa_pose_frequency,
            'GLSA_FOUR_JOINT_PAD': self.glsa_four_joint_pad,
            'GLSA_SAME_SIDE_PAD': self.glsa_same_side_pad,
            'POSE_REPO_PATH': self.pose_repo_path,
    ...
    """
    File structure:
    Video-Based_Infant_Respiration_Estimation/
        air-400/
            preprocess/
                pre
        Infant-Pose_Estimation/
    """

    def _init_pose_model(self) -> None:
        """Load FiDIP/HRNet model

        Expect config keys:
          POSE_REPO_PATH: /path/Infant-Pose-Estimation
          POSE_CFG_FILE: /path/experiments/coco/hrnet/w48_384x288_adam_lr1e-3_infant.yaml
          POSE_MODEL_PATH: /path/models/hrnet_fidip.pth
        """
        if not self.pose_repo_path or not self.pose_cfg_file or not self.pose_model_path:
            self.logger.warning(
                "DO_POSE_GUIDED_GLSA=True but pose paths are missing. "
                "GLSA fail, the model fall back to baseline."
            )
            return
        try:
            pose_root = os.path.abspath(self.pose_repo_path)
            pose_lib = os.path.join(pose_root, 'lib')
            
            # /path/Infant-Pose-Estimation/lib/adaptive_pose_hrnet.py
            # create adaptive_pose_hrnet module into memory
            if pose_lib in sys.path:
                sys.path.remove(pose_lib)
            if pose_root in sys.path:
                sys.path.remove(pose_root)
                
            sys.path.insert(0, pose_lib)
            sys.path.insert(0, pose_root)

            from config import cfg as pose_cfg, update_config as pose_update_config
            from utils.transforms import get_affine_transform

            import importlib.util

            adaptive_pose_hrnet_path = os.path.join(
                pose_lib,
                "models",
                "adaptive_pose_hrnet.py"
            )

            spec = importlib.util.spec_from_file_location(
                "fidip_adaptive_pose_hrnet",
                adaptive_pose_hrnet_path
            )

            adaptive_pose_hrnet = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(adaptive_pose_hrnet)

            # update_config expect cfg and arg
            # creating fake arg object for update_config
            class PoseArgs:
                cfg = os.path.join(pose_root, self.pose_cfg_file)
                opts = []
                modelDir = ""
                logDir = ""
                dataDir = ""
            ...
            pose_update_config(pose_cfg, PoseArgs())
            model_p, _ = adaptive_pose_hrnet.get_adaptive_pose_net(pose_cfg, is_train=False)
            #extract the pretrain weight
            raw = torch.load(os.path.join(pose_root, self.pose_model_path), map_location=self.device)
            if isinstance(raw, dict):
                for key in ("state_dict", "model", "net"):
                    if key in raw:
                        raw = raw[key]
                        break
            model_p.load_state_dict(raw, strict=False)
            self.pose_model = model_p.to(self.device).eval()
            self.pose_cfg = pose_cfg
            self.pose_get_affine_transform = get_affine_transform
            self.logger.info(f"FiDIP/HRNet  model being loaded from {self.pose_model_path}")
        except Exception:
            self.logger.exception("Failed to load FiDIP/HRNet  model; using fallback")
            self.pose_model = None

    
    def _pose_bbox_xywh_to_center_scale(self, x, y, w, h):
        """
        HRNet bbox preprocessing adapted from /Infant-Pose-Estimation/lib/dataset/coco.py
        Note: HRNet/COCO represents bbox size as normalized scale using /200
        """
        pixel_std = 200
        input_w, input_h = self.pose_cfg.MODEL.IMAGE_SIZE
    ...
    def _estimate_pose_keypoints(self, frame: np.ndarray) -> Optional[List[Tuple[int, int, float]]]:
        """Run FiDIP pose on one body-cropped frame and return 17 keypoints in crop coordinates."""
        if self.pose_model is None or self.pose_cfg is None or self.pose_get_affine_transform is None:
            return None
        try:
            # HRNet preprocessing
            # affine transform -> normalization -> tensor conversion -> HRNet inference
            img = frame.astype(np.uint8)
            H, W = img.shape[:2]
            center, scale = self._pose_bbox_xywh_to_center_scale(0, 0, W, H)
            trans = self.pose_get_affine_transform(center, scale, 0, self.pose_cfg.MODEL.IMAGE_SIZE)
            input_w, input_h = self.pose_cfg.MODEL.IMAGE_SIZE
            img_rgb = img if img.shape[-1] == 3 else cv2.cvtColor(img, cv2.COLOR_GRAY2RGB)
            
            warped = cv2.warpAffine(img_rgb, trans, (input_w, input_h), flags=cv2.INTER_LINEAR)
            warped = warped.astype(np.float32) / 255.0
            warped = (warped - np.array([0.485, 0.456, 0.406], dtype=np.float32)) / np.array([0.229, 0.224, 0.225], dtype=np.float32)
            
            tensor = torch.from_numpy(warped).permute(2, 0, 1).unsqueeze(0).float().to(self.device)
            with torch.no_grad():
                _, kpt_output = self.pose_model(tensor)
            heatmaps = kpt_output.detach().cpu().numpy()[0]
            ...
            for j in range(num_joints):
                hm = heatmaps[j]
                ky, kx = np.unravel_index(np.argmax(hm), hm.shape)
                conf = float(hm[ky, kx])
                x_input = kx / hm_w * input_w
                y_input = ky / hm_h * input_h
                pt = np.array([x_input, y_input, 1.0])
                x_orig = int(inv_trans[0] @ pt)
                y_orig = int(inv_trans[1] @ pt)
                keypoints.append((x_orig, y_orig, conf))
            return keypoints
        except Exception:
            self.logger.exception("Pose estimation failed on a frame")
            return None

    def _get_pose_torso_box_in_crop(self, frames: np.ndarray) -> Optional[List[int]]:
        """
        Runs joint detection every glsa_pose_frequency
        return if it detect same-side shoulder/hip combination or all four toros joint
        COCO toros joints: 5, 6, 11, 12
        the torso box are relative to the cropped body frame
        """
        T, H, W, _ = frames.shape
        step = max(1, self.glsa_pose_frequency)
        idxs = list(range(0, T, step))
        if not idxs or idxs[-1] != T - 1:
            idxs.append(T - 1)

        boxes = []
        torso_joint_ids = [5, 6, 11, 12]
        left_pair = [5, 11]
        right_pair = [6, 12]
        ...
        for i in idxs:
            ...
            kpts = self._estimate_pose_keypoints(frames[i])
            if not kpts:
                pose_failed_count += 1
                fallback_count += 1
                continue

            # Keep only confident shoulder/hip joints.
            valid = {
                j: (kpts[j][0], kpts[j][1], kpts[j][2])
                for j in torso_joint_ids
                if j < len(kpts) and kpts[j][2] >= self.glsa_pose_conf_threshold
            }
            
            # Check combination 
            has_all_four = all(j in valid for j in torso_joint_ids)
            has_left_pair = all(j in valid for j in left_pair)
            has_right_pair = all(j in valid for j in right_pair)

            if has_all_four:
                used_joint_ids = torso_joint_ids
                pad = self.glsa_four_joint_pad
                rule = "all_four_torso_joints"
                four_joint_count += 1
            else:
                candidates = []
                if has_left_pair:
                    score = valid[5][2] + valid[11][2]
                    candidates.append((score, left_pair, "left_same_side_pair"))
                if has_right_pair:
                    score = valid[6][2] + valid[12][2]
                    candidates.append((score, right_pair, "right_same_side_pair"))

                if not candidates:
                    fallback_count += 1
                    continue

                # If both same-side pairs are available but not all four are confident
                # use the side with higher confidence
                _, used_joint_ids, rule = max(candidates, key=lambda item: item[0])
                pad = self.glsa_same_side_pad
                same_side_count += 1

            pts = [(valid[j][0], valid[j][1]) for j in used_joint_ids]
            xs = [p[0] for p in pts]
            ys = [p[1] for p in pts]
            ...
            if x2 > x1 and y2 > y1:
                ...
                self.logger.debug(
                    "GLSA torso candidate frame=%s rule=%s pad=%.3f box=%s",
                    i, rule, pad, boxes[-1]
                )
            else:
                fallback_count += 1
        ...
        if total_checked > 0:
            self.logger.info(
                "GLSA stats: total=%d pose_failed=%d four_joint=%d same_side=%d fallback=%d",
                total_checked,
                pose_failed_count,
                four_joint_count,
                same_side_count,
        ...
        if boxes:
            return self._get_best_box(boxes, W, H, square=False)

        return None

    def _make_grid_attention_mask(self, shape_hw: Tuple[int, int], torso_box: Optional[Sequence[int]]) -> np.ndarray:
        """
        If no torso roi is found, use all-one mask(baseline)
        """
        H, W = shape_hw
        ...
        #set grid size and apply base weight
        grid_h, grid_w = int(self.glsa_grid_size[0]), int(self.glsa_grid_size[1])
        grid = np.full((grid_h, grid_w), self.glsa_base_weight, dtype=np.float32)

        tx, ty, tw, th = map(float, torso_box)
        tx2, ty2 = tx + tw, ty + th

        # y1, y2, x1, x2 is the boundaries of the grid cells
        for gy in range(grid_h):
            ...
            for gx in range(grid_w):
                ...
                # find the overlapping boundaries between current grid cell and the torso box
                ix1, iy1 = max(x1, tx), max(y1, ty)
                ix2, iy2 = min(x2, tx2), min(y2, ty2)

                inter = max(0.0, ix2 - ix1) * max(0.0, iy2 - iy1)
                cell_area = max(1.0, (x2 - x1) * (y2 - y1))
                overlap = inter / cell_area

                if overlap > 0:
                    grid[gy, gx] = self.glsa_base_weight + (
                        self.glsa_torso_weight - self.glsa_base_weight
                    ) * min(1.0, overlap * 2.0)

        mask = cv2.resize(grid, (W, H), interpolation=cv2.INTER_LINEAR).astype(np.float32)
        return mask[..., None]

    def build_pose_guided_glsa_mask(self, frames: np.ndarray) -> np.ndarray:
        torso_box = self._get_pose_torso_box_in_crop(frames)
        mask = self._make_grid_attention_mask(frames.shape[1:3], torso_box)

        if torso_box is None:
            self.logger.warning(
                "Pose-guided GLSA fallback to baseline: no reliable four-joint torso "
                "or same-side shoulder-hip pair found. Using all-one mask."
            )
        else:
            self.logger.info(f"Pose-guided GLSA torso box in cropped ROI: {torso_box}")

        return mask

    def apply_glsa_mask_to_flow(self, flow: np.ndarray, mask: Optional[np.ndarray]) -> np.ndarray:
        """Apply a precomputed GLSA mask to the single optical-flow result.

        The flow is computed on the cropped ROI. If the optical-flow backend changes
        resolution, resize the mask to match the flow resolution before multiplying.
        """
        ...
        if (mask_h, mask_w) != (flow_h, flow_w):
            resized = cv2.resize(mask[..., 0], (flow_w, flow_h), interpolation=cv2.INTER_LINEAR)
            mask = resized.astype(np.float32)[..., None]
    ...
    def apply_pose_guided_glsa(self, flow: np.ndarray, frames: np.ndarray) -> np.ndarray:
        """Backward-compatible wrapper: build a mask from cropped frames and apply it to flow."""
        if flow is None:
            return flow
        mask = self.build_pose_guided_glsa_mask(frames)
        return self.apply_glsa_mask_to_flow(flow, mask)
